...
def run_report(self, switch_key: str, account_name: str) -> dict:
    """Entry point called by FastAPI. Bridges sync Celery → async pipeline."""
    return asyncio.run(_async_run_report(self, switch_key, account_name))


# ------------------------------------------------------------------ async pipeline

async def _async_run_report(task, switch_key: str, account_name: str) -> dict:
    ...
    async with AkamaiClient(base_url, auth, sem) as client, \
        ...
        prop_tasks = [
            _process_property(task, client, reporting_client, redis, switch_key, g, prop, settings)
...
async def _process_property(task, client, reporting_client, redis, switch_key, group, prop_details, settings):
    ...
    try:
        # Rule tree + hostnames fetched concurrently
        rule_tree, hostnames_data = await asyncio.gather(
        ...
        traffic_data = {}
        if all_cpcodes_flat:
            resp = await reporting_client.get_traffic(switch_key, all_cpcodes_flat)
            if resp.get("data"):
