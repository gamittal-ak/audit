...
_HEADERS = [
    ...
    "Product", "CNAME From", "CNAME To", "Origin Characteristics", "SRO",
    "Site Shield", "Advanced Override", "Custom Override",
    "Custom Behavior Count", "CW/QR",
]
        ...
        for prop in properties:
            ...
            if not hostnames:
                for entry in _expand_cpcodes(cpcodes):
                    ws.append(base + entry + ["", ""])
                continue

            if not cpcodes:
                for h in hostnames:
                    ws.append(base + ["", "", ""] + _hostname_cols(h))
                continue

            for entry in _expand_cpcodes(cpcodes):
                for h in hostnames:
                    ws.append(base + entry + _hostname_cols(h))
...
def _prop_base(group_name, group_id, parent_group_id, contract_id, prop) -> list:
    ...
    return [
        ...
        # CP code columns (cpcode, desc, product) appended by caller
        # CNAME columns appended by caller
        # --- flags below ---
...
def _expand_cpcodes(cpcodes: list) -> list:
    """Expand each cpcode entry into a [cpcode, description, product] row fragment."""
    rows = []
